...
def plot_llama_property(x_column: str, y_column: str, AGN_data, inactive_data, agn_bol, inactive_bol, GB21, use_gb21=False, soloplot=None,exclude_names=None,logx=False,logy=False,background_image=None,manual_limits=None, legend_loc ='best' , truescale=False):
    ...
    fit_data_AGN = pd.read_csv('/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/AGN/gas_analysis_summary.csv')
    fit_data_inactive = pd.read_csv('/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/inactive/gas_analysis_summary.csv')
    ...
    print('fit_data_inactive')
    display(fit_data_inactive)

    print('agn_bol')
    display(agn_bol)
    print('inactive_bol')
    display(inactive_bol)
    ...
    print('merged_AGN')
    display(merged_AGN)
    print('merged_inactive')
    display(merged_inactive)
    ...
    # Plot scatter points
    if soloplot in (None, 'AGN'):
        ax_scatter.scatter(x_agn, y_agn, color='red', label='LLAMA AGN', marker='s')
        for x, y, name in zip(x_agn, y_agn, names_agn):
            ax_scatter.text(float(x + 0.005), float(y), name, fontsize=7, color='darkred',zorder=10)

    if soloplot in (None, 'inactive'):
        ax_scatter.scatter(x_inactive, y_inactive, color='blue', label='LLAMA Inactive', marker='v',zorder=10)
    ...
    if use_gb21 and soloplot is None:
        ax_scatter.scatter(x_gb21, y_gb21, color='green', label='GB21', marker='x',zorder=10)
        for x, y, name in zip(x_gb21, y_gb21, names_gb21):
            ax_scatter.text(float(x), float(y), name, fontsize=7, color='green',zorder=10)
...
# Histogram subplot
    ...
    output_path = f'/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/Plots/{x_column}_vs_{y_column}{suffix}.png'
    ...
    output_path = f'/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/Plots/pair_diffs/{y_column}_pair_differences.png'
    plt.savefig(output_path)
...
plot_llama_property('area_weighted_sd','mass_weighted_sd',AGN_data, inactive_data, agn_Rosario2018, inactive_Rosario2018,GB21_density,False,logx=True,logy=True,background_image='/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/Leroy2013_plots/Sigma.png',manual_limits=[0.5,5000,0.5,5000], truescale=True)