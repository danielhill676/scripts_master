...
def plot_llama_triptych(
    ...
    path_wis = f"/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/AGN/gas_analysis_summary_120pc_flux90_strict_1.5kpc.csv"
    path_phangs = f"/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/inactive/gas_analysis_summary_120pc_flux90_strict_1.5kpc.csv"
    ...
    if wis_df is not None:
        ...
        optional_datasets['AGN Gauss'] = wis_df
    if phangs_df is not None:
        ...
        optional_datasets['inactive Gauss'] = phangs_df
    ...
    def extract_xy(df, xcol, ycol):
        ...
        name_col = "Galaxy" if "Galaxy" in df.columns else "name" if "name" in df.columns else "Name"
    ...
    # Prepare panels
    # --------------------------------------------------
    datasets_for_plotting = {"AGN fft": fit_data_wis, "inactive fft": fit_data_phangs, **optional_datasets}
    panels = {}
    ...
    for key in ["left", "top", "bottom"]:
        ...
        x_w, y_w, n_w = panels[key]["AGN fft"]
        x_p, y_p, n_p = panels[key]["inactive fft"]
    ...
    default_label_styles = {
        "AGN fft":      ("s", "deepskyblue", 6),
        "inactive fft":  ("o", "firebrick",   6),
        "AGN Gauss":          ("H", "indigo",      7),
        'inactive Gauss':          ("D", "orange",      5),
    }
    ...
    if "AGN fft" in datasets_for_plotting:
        label_styles["AGN fft"] = default_label_styles["AGN fft"]
    if "inactive fft" in datasets_for_plotting:
        label_styles["inactive fft"] = default_label_styles["inactive fft"]

    # Always add optional datasets
    for opt in ['Comparison-control', 'AGN Gauss', 'inactive Gauss']:
    ...
    for key in panels:
        ...
        for label, (marker, color, size) in label_styles.items():
            ...
            # --------------------------------------------------
            if label in ["AGN Gauss", 'inactive Gauss']: #### edited to print out all names
    ...
    if not connect:
        plt.savefig(f"{outfolder}/AGN Gauss_CAS_triptych_gauss_AGN_inactive.png", dpi=300)
    else:
        plt.savefig(f"{outfolder}/AGN Gauss_CAS_triptych_connect_shared.png", dpi=300)
...
agn_gauss_df = pd.read_csv("/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/AGN_gauss/gas_analysis_summary_120pc_flux90_strict_1.5kpc.csv")
inactive_gauss_df = pd.read_csv("/Users/administrator/Astro/LLAMA/ALMA/gas_distribution_fits/inactive_gauss/gas_analysis_summary_120pc_flux90_strict_1.5kpc.csv")
